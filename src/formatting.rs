...
pub fn format_default(result: &MultipleSearchResult) -> String {
    format!("{}: {}", result.line_no, result.line)
}

pub fn format_minimal(result: &MultipleSearchResult) -> String {
    format!("{}", result.line)
}

pub fn format_verbose(result: &SearchResult) -> String {
    let prefix = format!("{}: ", result.line_no);
    let pad = result.column + prefix.len() + 1;
    format!("{}{}\n{:>pad$}", prefix, result.line, "^")
}
